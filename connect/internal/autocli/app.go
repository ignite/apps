...
type AppOptions struct {
	...
	ValidatorAddressCodec address.Codec // ValidatorAddressCodec is used to encode/decode validator addresses.
	ConsensusAddressCodec address.Codec // ConsensusAddressCodec is used to encode/decode consensus addresses.
...
func (appOptions AppOptions) EnhanceRootCommand(rootCmd *cobra.Command) error {
	// convert the API sign modes to internal sign modes
	signModes := make([]apisigning.SignMode, len(appOptions.TxConfigOpts.EnabledSignModes))
	for i, mode := range appOptions.TxConfigOpts.EnabledSignModes {
		signModes[i] = apisigning.SignMode(mode)
	}
