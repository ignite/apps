	...
	if heights := md.Get(grpcBlockHeightHeader); len(heights) > 0 {
		...
		if height < 0 {
			return errorsmod.Wrapf(
				...
				"Invoke: height (%d) from %q must be >= 0", height, grpcBlockHeightHeader)
		}
	}

	abciR := abci.RequestQuery{
		Path:   method,
		Data:   reqBz,
		Height: height,
	}
...
func (c *CometBFTBroadcaster) queryABCI(ctx context.Context, req abci.RequestQuery) (abci.ResponseQuery, error) {
