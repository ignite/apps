...
func NewFactoryFromFlagSet(f *pflag.FlagSet, keybase keyring.Keyring, cdc codec.BinaryCodec, accRetriever account.AccountRetriever,
	txConfig TxConfig, ac address.Codec, conn gogogrpc.ClientConn,
) (Factory, error) {
	offline, _ := f.GetBool(FlagOffline)
	if err := validateFlagSet(f, offline); err != nil {
		return Factory{}, err
	}

	params, err := txParamsFromFlagSet(f, keybase, ac)
	if err != nil {
		return Factory{}, err
	}
...
func validateFlagSet(f *pflag.FlagSet, offline bool) error {
	dryRun, _ := f.GetBool(FlagDryRun)
	if offline && dryRun {
		return errors.New("dry-run: cannot use offline mode")
	}

	generateOnly, _ := f.GetBool(FlagGenerateOnly)
	chainID, _ := f.GetString(FlagChainID)
	if offline {
		if !generateOnly && (!f.Changed(FlagAccountNumber) || !f.Changed(FlagSequence)) {
...
func (f *Factory) BuildUnsignedTx(msgs ...sdk.Msg) error {
...
func (f *Factory) BuildsSignedTx(ctx context.Context, msgs ...sdk.Msg) (Tx, error) {
...
func (f *Factory) calculateGas(msgs ...sdk.Msg) error {
...
func (f *Factory) Simulate(msgs ...sdk.Msg) (*apitx.SimulateResponse, uint64, error) {
...
// printed.
func (f *Factory) UnsignedTxString(msgs ...sdk.Msg) (string, error) {
...
// msgsV1toAnyV2 converts a slice of sdk.Msg (v1) to a slice of anypb.Any (v2).
// It first converts each sdk.Msg into a codectypes.Any and then converts
