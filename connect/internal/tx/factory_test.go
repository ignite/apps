...
var (
	FromAddress = "cosmos1zglwfu6xjzvzagqcmvzewyzjp9xwqw5qwrr8n9"
	addr, _     = ac.StringToBytes(FromAddress)
)
...
func TestFactory_getSignBytesAdapter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			FromAddressData := signing.SignerData{
				Address:       addr,
			...
			got, err := f.getSignBytesAdapter(context.Background(), FromAddressData)
			if tt.error {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			txSetter: func(f *Factory) {
				...
				f.tx.msgs = []sdk.Msg{&banktypes.MsgSend{
					FromAddress: addr,
				}}
...
func TestFactory_setMsgs(t *testing.T) {
	tests := []struct {
		name    string
		msgs    []sdk.Msg
		wantErr bool
	}{
		{
			...
			msgs: []sdk.Msg{
				&banktypes.MsgSend{
					FromAddress: "cosmos1zglwfu6xjzvzagqcmvzewyzjp9xwqw5qwrr8n9",
					ToAddress:   "cosmos1zglwfu6xjzvzagqcmvzewyzjp9xwqw5qwrr8n9",
				},
				&banktypes.MsgSend{
					FromAddress: "cosmos1zglwfu6xjzvzagqcmvzewyzjp9xwqw5qwrr8n9",
					ToAddress:   "cosmos1zglwfu6xjzvzagqcmvzewyzjp9xwqw5qwrr8n9",
