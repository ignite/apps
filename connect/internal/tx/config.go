...
func newHandlerMap(opts ConfigOptions, signingCtx *signing.Context) (hm *signing.HandlerMap, err error) {
	lenSignModes := 3
	handlers := make([]signing.SignModeHandler, lenSignModes+len(opts.CustomSignModes))

	// sign mode direct
	handlers[0] = &direct.SignModeHandler{}
	// sign mode direct aux
	handlers[1], err = directaux.NewSignModeHandler(directaux.SignModeHandlerOptions{
	...
	handlers[2] = aminojson.NewSignModeHandler(aminojson.SignModeHandlerOptions{
	...
	hm = signing.NewHandlerMap(handlers...)
