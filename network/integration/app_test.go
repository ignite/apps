...
func TestWasm(t *testing.T) {
	var (
		...
		env         = envtest.New(t)
		app         = env.Scaffold("github.com/apps/networkapp")
	...
	dir, err := os.Getwd()
	require.NoError(err)
	pluginPath := filepath.Join(filepath.Dir(filepath.Dir(dir)), "network")

	env.Must(env.Exec("install network app locally",
